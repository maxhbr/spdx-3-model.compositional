...
instance ToJSON IntegrityMethod where
  toJSON (Hash comment algorithm hashValue) = object
    ["comment" .= comment, "algorithm" .= algorithm, "hashValue" .= hashValue]
instance FromJSON IntegrityMethod where
  parseJSON = withObject "IntegrityMethod" $ \o -> do
    Hash <$> o .:? "comment" <*> o .: "algorithm" <*> o .: "hashValue"
...
emptyElement =
  ElementProperties Nothing Nothing Nothing Nothing mempty mempty mempty
elementFromName :: String -> Element
elementFromName name =
  ElementProperties (Just name) Nothing Nothing Nothing mempty mempty mempty
...
instance ToJSON Artifact where
  toJSON (ArtifactProperties originatedBy) =
    object ["originatedBy" .= originatedBy]
...
instance ToJSON Bundle where
  toJSON (BundleProperties context) = object ["context" .= context]
...
instance ToJSON Relationship where
  toJSON (RelationshipProperties t from to relationshipCompleteness) = object
    [ "relationshipType" .= t
    , "from" .= from
    , "to" .= to
...
instance ToJSON Annotation where
  toJSON (AnnotationProperties statement subject) =
    object ["statement" .= statement, "subject" .= subject]
...
instance ToJSON File where
  toJSON (FileProperties contentIdentifier filePurpose contentType) = object
    [ "contentIdentifier" .= contentIdentifier
...
instance ToJSON (SPDX a) where
  ...
  toJSON e =
    let t = object ["@type" .= getType e]
        mergeObjects :: [Value] -> Value
        mergeObjects list =
          let unObject :: Value -> Object
              unObject (Object o) = o
        ...
        getJsons (Element i ci e) =
          [object ["SPDXID" .= i, "creationInfo" .= ci], toJSON e]
...
mkExample = do
  let actors =
        [ Actor (Just "Some Actor") (Just PERSON)
        , Actor (Just "This Tool")  (Just TOOL)
        ]
  creationInfo <- mkCreationInfo actors
  return . runSPDX creationInfo $ collection "urn:spdx:Collection0" $ do
